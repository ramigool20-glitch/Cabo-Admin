...
import { RegistrarPagoForm } from '@/components/por-pagar/registrar-pago-form'
import { cancelarCuenta } from '@/app/(app)/por-pagar/actions'
import { EliminarCuentaPagarBtn } from '@/components/por-pagar/eliminar-btn'
...
export default async function DetalleCuentaPorPagarPage(
  ...
  const [{ data: cuenta }, { data: pagos }, { data: cuentas }] = await Promise.all([
    supabase.from('cuentas_por_pagar').select('*, negocios(nombre)').eq('id', id).single(),
    supabase.from('cuentas_por_pagar_pagos').select('*, cuentas(nombre)').eq('cuenta_id', id).order('fecha_pago', { ascending: false }),
  ...
  const vencido = cuenta.estado !== 'pagado' && cuenta.fecha_vencimiento && cuenta.fecha_vencimiento < hoy
    ...
    <div className="px-4 pt-4 pb-24 space-y-5 max-w-2xl mx-auto">
      <div className="flex items-center justify-between gap-2">
        ...
        <div className="flex items-center gap-2">
          ...
          <EliminarCuentaPagarBtn id={id} proveedor={cuenta.proveedor} />
        </div>
      </div>
      ...
      <header className="space-y-2">
        <div className="flex items-start justify-between gap-3">
          <div className="flex-1 min-w-0">
            <h1 className="text-2xl font-bold text-white">{cuenta.proveedor}</h1>
      ...
      <div className="card-glow p-5 space-y-2">
        <div className="flex items-baseline justify-between">
          ...
          <p className={cn('text-3xl font-black tabular-nums', restante > 0 ? 'text-rose-300' : 'text-emerald-300')}>
        ...
        {Number(cuenta.monto_pagado) > 0 && (
          <div className="flex items-baseline justify-between text-sm">
            ...
            <span className="text-emerald-400 tabular-nums">{formatMoney(Number(cuenta.monto_pagado), cuenta.moneda as 'MXN' | 'USD')}</span>
          </div>
        )}
        <div className="flex items-baseline justify-between text-sm">
        ...
        {Number(cuenta.monto_total) > 0 && (
          <div className="pt-2">
            ...
            <p className="text-[10px] text-zinc-500 mt-1 text-right">
              {Math.round((Number(cuenta.monto_pagado) / Number(cuenta.monto_total)) * 100)}% pagado
            </p>
          </div>
        )}
      </div>
      ...
      <div className="card p-4 space-y-2 text-sm">
        ...
        {cuenta.proveedor_telefono && (
          <div className="flex justify-between">
            ...
            <a href={`tel:${cuenta.proveedor_telefono}`} className="text-cyan-400">{cuenta.proveedor_telefono}</a>
          </div>
        )}
        {cuenta.proveedor_email && (
          <div className="flex justify-between">
            <span className="text-zinc-500 inline-flex items-center gap-1"><Mail className="h-3 w-3" /> Email</span>
            <a href={`mailto:${cuenta.proveedor_email}`} className="text-cyan-400">{cuenta.proveedor_email}</a>
          </div>
        )}
      </div>

      {/* Registrar pago */}
      {cuenta.estado !== 'pagado' && cuenta.estado !== 'cancelado' && (
        <section className="card-glow p-4 space-y-3">
          <p className="label-caps">Registrar pago</p>
          <RegistrarPagoForm
            cuentaId={id}
            restante={restante}
      ...
      {pagos && pagos.length > 0 && (
        <section className="space-y-2">
          <h2 className="label-caps">Pagos ({pagos.length})</h2>
              ...
              return (
                <li key={p.id} className="p-3 flex items-center justify-between">
                  <div className="leading-tight">
                    <p className="text-sm font-medium text-white">{formatearFecha(p.fecha_pago, 'dd MMM yyyy')}</p>
      ...
      {/* Cancelar */}
      {cuenta.estado !== 'cancelado' && cuenta.estado !== 'pagado' && (
        <form action={cancelarCuenta.bind(null, id)}>
          <button
            type="submit"
